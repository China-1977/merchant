...
create unique index if not exists ui_vip_store_id_account_id on master.vip (store_id, account_id);
...
comment on column master.site.postcode is '邮编';
comment on column master.site.code is '省市区编号';
comment on column master.site.value is '省市区';
comment on column master.site.insert_time is '创建时间';
comment on column master.site.update_time is '更新时间';
